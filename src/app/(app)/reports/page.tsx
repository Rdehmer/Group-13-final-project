...
const CONTRACT_VISIT_COST = 350;

function toRow(c: ServiceContract): ContractRow {
  const rev = Number(c.contract_price);
  const cost = c.included_service_visits * CONTRACT_VISIT_COST;
...
export default function ReportsPage() {
  ...
  const activeContractDirectCost = activeContracts.reduce(
    (s, c) => s + c.included_service_visits * CONTRACT_VISIT_COST,
    0,
  );
  ...
  const contractRows = isManager ? activeContractRows : allContractRows;
  ...
  function InactiveContractTable({ rows }: { rows: ContractRow[] }) {
    if (rows.length === 0) {
      return <EmptyState title="No non-active contracts" description="Matching contracts will appear here." />;
    }
    return (
    ...
    <div>
      ...
      <div className="mb-4 rounded-box bg-base-100 p-4 text-sm shadow">
        ...
        <ul className="mt-2 list-inside list-disc opacity-80">
          ...
          <li>Contract visit cost estimated at ${CONTRACT_VISIT_COST} per included visit</li>
          {isManager ? (
            <li>
              Contract Est. Direct Cost / Est. Gross Margin use Active contracts only (same as
              Contracts tab)
            </li>
          ) : null}
        </ul>
      ...
      {isManager ? (
        <>
          <div className="mb-2 text-sm font-medium opacity-70">
            Contract summary (matches Contracts tab)
          </div>
          <div className="mb-6 grid gap-4 sm:grid-cols-3">
            ...
            <SegmentButton
              ...
              hint={`Assumes $${CONTRACT_VISIT_COST}/visit avg`}
            />
            <SegmentButton
              column="margin"
      ...
      <div className={isManager ? "mb-2 text-sm font-medium opacity-70" : "hidden"}>
        Invoice & cash summary
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          label={isManager ? "Invoice Est. Gross Margin" : "Est. Gross Margin"}
      ...
      <div id="contract-profitability" className="mt-6 card bg-base-100 shadow">
        <div className="card-body">
          ...
          {contractRows.length === 0 ? (
            <EmptyState
              title={isManager ? "No active contracts" : "No contract data"}
              description="Matching contracts will appear in profitability tables."
            />
          ) : (
            <div className="overflow-x-auto">
              <table className="table">
                <thead>
                  <tr>
                    <th>Contract</th>
                    <th className={isManager && highlight === "revenue" ? "bg-warning/30" : ""}>
                      Revenue
                    </th>
                    <th className={isManager && highlight === "cost" ? "bg-warning/30" : ""}>
                      Est. Cost
                    </th>
                    <th>Est. Profit</th>
                    <th className={isManager && highlight === "margin" ? "bg-warning/30" : ""}>
                      Margin
                    </th>
                    <th>Status</th>
                  </tr>
                ...
                <tbody>
                  {contractRows.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td
                        className={
                          isManager && highlight ? "bg-base-100 text-base-content/50" : ""
                        }
                      >
      ...
      {isManager ? (
        <div className="mt-6 card bg-base-100 shadow">
          <div className="card-body">
            ...
            <InactiveContractTable rows={inactiveContractRows} />
          </div>
        </div>
      ) : null}
