...
export default function TechnicianPage() {
  ...
  async function loadWorkOrders(techId?: string, role?: Profile["role"]) {
    let query = supabase.from("work_orders").select("*, customers(name)").not("status", "in", '("Completed","Closed","Canceled")').order("scheduled_date");
            ...
            <>
                ...
                <div className="card-body">
                  ...
                  {labor.length > 0 ? (
                    <table className="table table-sm mt-4">
                      <thead><tr><th>Date</th><th>Regular</th><th>OT</th><th>Notes</th></tr></thead>
                      <tbody>{labor.map((l) => <tr key={l.id}><td>{l.work_date}</td><td>{l.regular_hours}</td><td>{l.overtime_hours}</td><td>{l.notes ?? "—"}</td></tr>)}</tbody>
                    </table>
                  ) : null}
                </div>
              </div>
              ...
              <div className="card bg-base-100 shadow">
                <div className="card-body">
                  ...
                  {parts.length > 0 ? (
                    <table className="table table-sm mt-4">
                      <thead><tr><th>Part</th><th>Qty</th><th>Billable</th></tr></thead>
                      <tbody>{parts.map((p) => <tr key={p.id}><td>{p.parts?.name ?? p.part_id}</td><td>{p.quantity_used}</td><td>${Number(p.billable_amount).toFixed(2)}</td></tr>)}</tbody>
                    </table>
                  ) : null}
                </div>
              </div>
              ...
              <div className="card bg-base-100 shadow">
                <div className="card-body">
                  ...
                  <form onSubmit={addAdditionalWork} className="mt-2 space-y-3">
                    ...
                    <FormRow label="Est. charge"><input type="number" min="0" step="0.01" className="input input-bordered w-full" value={awrForm.estimated_additional_charge} onChange={(e) => setAwrForm({ ...awrForm, estimated_additional_charge: e.target.value })} /></FormRow>
