...
Customer **Payments** (`/customer/pay`) uses Stripe test-mode checkout when `STRIPE_SECRET_KEY` and `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` are set. Set `STRIPE_DEMO_MODE=true` only for simulated checkout without keys.
